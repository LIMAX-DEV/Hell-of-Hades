...
try:
    ...
    print(f'Modules of the python library required for LIMAX TOOLS are not installed, make sure you have correctly installed python and have launched the "Setup.py" file which will install all the necessary modules.')
    input(f'Error: {e}')

color_webhook = 0x0505a8  
...
tool_path = os.path.dirname(os.path.abspath(__file__)).split("программа\\")[0].split("программа/")[0].strip()
...
def Reset():
    if os_name == "Windows":
        file = ['python', os.path.join(tool_path, "базовый.py")]
        subprocess.run(file)

    elif os_name == "Linux":
        file = ['python3', os.path.join(tool_path, "базовый.py")]
        subprocess.run(file)

def StartProgram(program):
    if os_name == "Windows":
        file = ['python', os.path.join(tool_path, "программа", program)]
        subprocess.run(file)
        
    elif os_name == "Linux":
        file = ['python3', os.path.join(tool_path, "программа", program)]
        subprocess.run(file)
...
def MainColor(text):
    start_color = (5, 5, 168)  # Changed from (168, 5, 5) to dark blue
    end_color = (118, 118, 255)  # Changed from (255, 118, 118) to light blue
...
def ChoiceUserAgent():
    file_user_agent = os.path.join(tool_path, "2-ввод", "Headers", "UserAgent.txt")
...
def ChoiceMultiTokenDisord():
    ...
    file_token_discord_relative = "\\2-ввод\\TokenDisc\\TokenDisc.txt"
    file_token_discord = os.path.join(tool_path, "2-ввод", "TokenDisc", "TokenDisc.txt")
    tokens = {}
