...
try:
 ...
 {purple + PREFIX}nuke [Channels Number], [Channels Name], [Message Spam]
 {white}Delete all channels and create other channels and spam messages.
 {purple + PREFIX}spam_channels [Channels Number], [Channels Name], [Message Spam]
 {white}Created channels that spam messages.
 {purple + PREFIX}delete_channels
 {white}Delete all channels from the server.
 {purple + PREFIX}stop_message_spam
 {white}Stop all messages that are being spammed.
 {purple + PREFIX}send_pm [Message]
    ...
    async def nuke(ctx, *, args):
        ...
        for channel in guild.channels:
            try:
                ...
                print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Channel Not Delete: {white}{channel.name} ({channel.id}) {purple}Error: {white}{e}")
    ...
    async def spam_channel(channel):
        ...
        while spamming:
            try:
                ...
                print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Message Not Send: {white}{message_spam} {purple}Error: {white}{e}")
        ...
        async for member in guild.fetch_members(limit=None):
            if member != ctx.author:
                try:
                    ...
                    print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} {purple}Status: {white}Failed (Access denied) {purple}User: {white}{member.name}#{member.discriminator} ({member.id}) {purple}Message: {white}{message}")
                except Exception as e:
                    print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} {purple}Status: {white + e} {purple}User: {white}{member.name}#{member.discriminator} ({member.id}) {purple}Message: {white}{message}")
