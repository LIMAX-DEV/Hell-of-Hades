...
try:
    ...
    def DeleteFriends(friends, token):
        for friend in friends:
            try:
                ...
                print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Status: {white}Delete{purple} | User: {white}{friend['user']['username']}#{friend['user']['discriminator']}")
            except Exception as e:
                print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Status: {white}Error: {e}{purple}")
    ...
    if not requests.get("https://discord.com/api/v9/users/@me/relationships", headers={'Authorization': token, 'Content-Type': 'application/json'}).json():
        print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Status: {white}Error{purple}")
