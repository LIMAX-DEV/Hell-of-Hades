    ...
    while True:
        for i in range(len(statues)):
            ...
            try:
                ...
                print(f"{BEFORE + current_time_hour() + AFTER} {ADD} Status: {white}Changed{purple} Status Discord: {white}{statues[i]}{purple}")
                i += 1
                time.sleep(5)
            except Exception as e:
                print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Status: {white}Changed{purple} Status Discord: {white}{statues[i]}{purple}")
                time.sleep(5)
except Exception as e:
    Error(e)