...
try:      
 ...
 {BEFORE}03{AFTER}{white} Firefox (Windows)
    ...
    if browser in ['1', '01']:
        try:
            navigator = "Chrome"
            print(f"{BEFORE + current_time_hour() + AFTER} {WAIT} {navigator} Starting..{purple}")
            driver = webdriver.Chrome()
            print(f"{BEFORE + current_time_hour() + AFTER} {INFO} {navigator} Ready !{purple}")
        except:
    ...
    try:
        ...
        print(f"{BEFORE + current_time_hour() + AFTER} {WAIT} Token Connection..{purple}")
        driver.execute_script(script + f'\nlogin("{token}")')
        time.sleep(4)
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Connected Token !{purple}")
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} If you leave the tool, edge will close!{purple}")
        Continue()
        Reset()
    except:
        print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} {navigator} not installed or driver not up to date.")
