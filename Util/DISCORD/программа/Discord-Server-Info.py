from Полезный.cfg import *
from Полезный.Config import *

try:
    import requests
except Exception as e:
   ErrorModule(e)
   
Title("Discord Server Info")

try:
    Slow(discord_banner)
    invite = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Server Invitation -> {reset}")
    try:
        invite_code = invite.split("/")[-1]
    except:
        invite_code = invite

    response = requests.get(f"https://discord.com/api/v9/invites/{invite_code}")

    if response.status_code == 200:
        api = response.json()

        type_value = api.get('type', "None")
        code_value = api.get('code', "None")
        inviter_info = api.get('inviter', {})
        inviter_id = inviter_info.get('id', "None")
        inviter_username = inviter_info.get('username', "None")
        inviter_avatar = inviter_info.get('avatar', "None")
        inviter_discriminator = inviter_info.get('discriminator', "None")
        inviter_public_flags = inviter_info.get('public_flags', "None")
        inviter_flags = inviter_info.get('flags', "None")
        inviter_banner = inviter_info.get('banner', "None")
        inviter_accent_color = inviter_info.get('accent_color', "None")
        inviter_global_name = inviter_info.get('global_name', "None")
        inviter_banner_color = inviter_info.get('banner_color', "None")
        expires_at = api.get('expires_at', "None")
        flags = api.get('flags', "None")
        server_info = api.get('guild', {})
        server_id = server_info.get('id', "None")
        server_name = server_info.get('name', "None")
        server_icon = server_info.get('icon', "None")
        server_features = server_info.get('features', "None")
        if server_features != "None":
            server_features = ' / '.join(server_features)
        server_verification_level = server_info.get('verification_level', "None")
        server_nsfw_level = server_info.get('nsfw_level', "None")
        server_descritpion = server_info.get('description', "None")
        server_nsfw = server_info.get('nsfw', "None")
        server_premium_subscription_count = server_info.get('premium_subscription_count', "None")
        channel_info = api.get('channel', {})
        channel_id = channel_info.get('id', "None")
        channel_type = channel_info.get('type', "None")
        channel_name = channel_info.get('name', "None")
    else:
        print(f"{BEFORE + current_time_hour() + AFTER} {ERROR} Invalid URL or Invitation Code !", reset)
        time.sleep(3)
        Reset()

    Slow(f"""
{purple}Invitation Information:
{white}────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 {INFO_ADD} Invitation         : {white}{invite}{purple}
 {INFO_ADD} Type               : {white}{type_value}{purple}
 {INFO_ADD} Code               : {white}{code_value}{purple}
 {INFO_ADD} Expired            : {white}{expires_at}{purple}
 {INFO_ADD} Server ID          : {white}{server_id}{purple}
 {INFO_ADD} Server Name        : {white}{server_name}{purple}
 {INFO_ADD} Channel ID         : {white}{channel_id}{purple}
 {INFO_ADD} Channel Name       : {white}{channel_name}{purple}
 {INFO_ADD} Channel Type       : {white}{channel_type}{purple}
 {INFO_ADD} Server Description : {white}{server_descritpion}{purple}
 {INFO_ADD} Server Icon        : {white}{server_icon}{purple}
 {INFO_ADD} Server Features    : {white}{server_features}{purple}
 {INFO_ADD} Server NSFW Level  : {white}{server_nsfw_level}{purple}
 {INFO_ADD} Server NSFW        : {white}{server_nsfw}{purple}
 {INFO_ADD} Flags              : {white}{flags}{purple}
 {INFO_ADD} Server Verification Level         : {white}{server_verification_level}{purple}
 {INFO_ADD} Server Premium Subscription Count : {white}{server_premium_subscription_count}{purple}
{white}────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
""")

    if inviter_info:
        Slow(f"""{purple}Inviter Information:
{white}────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
 {INFO_ADD} ID            : {white}{inviter_id}{purple}
 {INFO_ADD} Username      : {white}{inviter_username}{purple}
 {INFO_ADD} Global Name   : {white}{inviter_global_name}{purple}
 {INFO_ADD} Avatar        : {white}{inviter_avatar}{purple}
 {INFO_ADD} Discriminator : {white}{inviter_discriminator}{purple}
 {INFO_ADD} Public Flags  : {white}{inviter_public_flags}{purple}
 {INFO_ADD} Flags         : {white}{inviter_flags}{purple}
 {INFO_ADD} Banner        : {white}{inviter_banner}{purple}
 {INFO_ADD} Accent Color  : {white}{inviter_accent_color}{purple}
 {INFO_ADD} Banner Color  : {white}{inviter_banner_color}{purple}
{white}────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
""")
    Continue()
    Reset()
except Exception as e:
    Error(e)